...
def plot_traffic_heatmap(config):
    ...
    plt.imshow(GLOBAL_BASEMAP_IMG, extent=GLOBAL_BASEMAP_EXTENT, zorder=0)
    sc = plt.scatter(
        lons, lats, c=speeds, cmap=heat_cfg.get(
            "cmap", "RdYlGn"), s=heat_cfg.get(
            "s", 20), alpha=heat_cfg.get(
                "alpha", 0.7), zorder=1)
    ...
    out_dir = os.getenv("RUN_IMAGE_DIR") or os.path.join(RUN_DIR or ".", "images")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.getenv("RUN_IMAGE_FILE") or os.path.join(
        out_dir, os.path.basename(heat_cfg.get("save_path", "traffic_heatmap.png")))
...
def plot_events(config):
    ...
    plt.scatter(
        ...
        c=ev_cfg.get(
            ...
            "#ff0000"),
        alpha=0.6,
        label=event.get("title") if ev_cfg.get("legend") else None)
    if ev_cfg.get("annotate"):
    plt.annotate(
        event.get(
            "title", "event"), (lon, lat), textcoords="offset points", xytext=(
            5, 5), ha="left", fontsize=8)
...
def main():
    ...
    parser.add_argument(
        "--run-dir",
        help="Run directory to read data from and write images into (overrides RUN_DIR env)")
