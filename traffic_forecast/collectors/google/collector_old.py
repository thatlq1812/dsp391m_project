    ...
    try:
        ...
        if data['status'] == 'OK' and data['routes']:
            ...
            if real_directions_api._call_count % 50 == 0:
                print(
                    f"API call #{real_directions_api._call_count} successful in {response_time:.2f}s: avg {speed_kmh:.1f} km/h")
        ...
        print(
            f"Google API timeout ({time.time() - start_time:.2f}s) for {origin['lat']},{origin['lon']} -> {dest['lat']},{dest['lon']}, falling back to mock")
        return None
...
def run_google_collector():
    ...
    if use_real_api:
        print(
            f"Rate limiting: {rate_limiter.requests_per_minute} requests/minute (~{rate_limiter.requests_per_second:.1f} req/sec)")
