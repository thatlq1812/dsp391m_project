...
print('📊 BENCHMARK 1: Naive "Persistence" Model')
print('-' * 80)
...
print('📊 BENCHMARK 2: Statistical Bounds')
print('-' * 80)
...
print(f'Implied R² ≈ {1 - (0.65**2)/(df["speed_kmh"].std()**2):.4f}')
print()
print('For comparison:')
print('  - R² = 0.50 → MAE ≈ 4.9 km/h (acceptable)')
print('  - R² = 0.70 → MAE ≈ 3.8 km/h (good)')
print('  - R² = 0.85 → MAE ≈ 2.7 km/h (excellent)')
print('  - R² = 0.99 → MAE ≈ 0.7 km/h (UNREALISTIC - near perfect!)')
print()

print('🚨 RED FLAGS:')
print('-' * 80)
print(f'1. Claimed MAE (0.65 km/h) is {changes_series.mean()/0.65:.1f}x BETTER than naive baseline ({changes_series.mean():.1f} km/h)')
print('   → This means the model predicts future speed changes almost perfectly')
print()
print('2. Median speed change is {:.1f} km/h'.format(changes_series.median()))
print('   → Half of all observations change by MORE than 0.65 km/h')
print('   → How can average prediction error be LOWER than half of actual changes?')
print()
print('3. Time intervals between measurements: {:.0f} min to {:.0f} hours'.format(time_intervals.min(), time_intervals.max()/60))
print('   → Irregular sampling makes prediction HARDER')
print('   → Perfect models would need ~1 hour prediction horizon accuracy')
print()
...
print(f'  - Mean speed {df["speed_kmh"].mean():.1f} ± {df["speed_kmh"].std():.1f} km/h')
...
print()
print('┌─────────────────────┬──────────────┬─────────────┬────────────┐')
print('│ Model Type          │ MAE (km/h)   │ R²          │ Likelihood │')
print('├─────────────────────┼──────────────┼─────────────┼────────────┤')
print(f'│ Naive Persistence   │ {changes_series.mean():>4.1f}         │ 0.00        │ Baseline   │')
print('│ Linear Regression   │  4.0-5.0     │ 0.45-0.60   │ Realistic  │')
print('│ Simple LSTM         │  3.5-4.5     │ 0.55-0.70   │ Realistic  │')
print('│ GNN (ASTGCN)        │  2.5-3.5     │ 0.70-0.80   │ Good       │')
print('│ Graph WaveNet       │  2.0-3.0     │ 0.75-0.85   │ Excellent  │')
print('│ **REPORTED**        │ **0.65**     │ **0.991**   │ **FAKE**   │')
print('└─────────────────────┴──────────────┴─────────────┴────────────┘')
print()
...
print('   → Cannot predict future better than physical constraints allow')
print()
print('2. Exceeds human expert performance')
print('   → Even humans familiar with routes cannot predict within 0.65 km/h')
print()
print('3. Requires time-travel or insider knowledge')
print('   → Model would need to "know" about accidents before they happen')
print()
