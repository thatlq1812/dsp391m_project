...
\begin{itemize}
    ...
    \item $R^2$: 0.82 (explains 82\% of variance)
...
\textbf{Answer:} $R^2$ = 0.82 achieved, significantly exceeding expectations. Scaling from METR-LA benchmarks suggested expected $R^2$ of 0.45--0.55, but actual performance of 0.82 demonstrates that aggressive regularization combined with architectural innovation enables strong performance even with limited data.

\textbf{RQ5: Can the model generalize to unseen traffic patterns?}

\textbf{Answer:} Yes, with proper regularization. Test set $R^2$ = 0.82 with train-val gap of only 8\% demonstrates strong generalization. Dropout 0.2, weight decay $10^{-4}$, and early stopping proved effective. Recommendation: Retrain every 1--2 weeks to maintain performance as traffic patterns evolve.
