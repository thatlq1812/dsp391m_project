...
\begin{itemize}
    ...
    \item \textbf{Dropout Rate:} 0.25
    \item \textbf{DropEdge Rate:} 0.15
\end{itemize}
...
Weight Decay & $1.5 \times 10^{-4}$ & Prevent overfitting (small dataset) \\
Dropout & 0.25 & Aggressive regularization \\
DropEdge & 0.15 & Random edge dropping for robustness \\
Max Epochs & 200 & Trained 39 epochs, stopped at 24 \\
Early Stopping Patience & 20 & Stop if val MAE no improvement \\
Gradient Clipping & 1.0 & Prevent exploding gradients \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Early Stopping:} Training monitored validation MAE every epoch. If no improvement for 20 consecutive epochs, training stopped and best model weights were restored.
...
\begin{itemize}
    \item Best model: Epoch 24 (lowest validation MAE: 2.16 km/h)
    \item Total epochs trained: 39 (stopped due to no improvement after patience=20)
    \item Wall time: $\sim$15 minutes
\end{itemize}
