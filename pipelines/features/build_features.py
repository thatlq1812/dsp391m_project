...
def load_weather():
    if os.path.exists('data/weather_snapshot.json'):
        with open('data/weather_snapshot.json', 'r') as f:
            return json.load(f)
    return []

def load_forecast():
    if os.path.exists('data/weather_forecast.json'):
...
def build_features_v2():
    ...
    with open('data/features_nodes_v2.json', 'w') as f:
